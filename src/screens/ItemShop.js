...
import { AsyncStorage, View, Text, Image, ActivityIndicator, ImageBackground, Dimensions } from 'react-native';
...
export default class ItemShop extends React.Component {
    ...
    _checkUpdate = async () => {
        ...
        if (data) {
            ...
            if (_data.update > Date.now()) {
                return this.setState({ itemShop: _data.data });
            }
        }
        this.fetchStore();
    }
    ...
    fetchStore = () => {
        fortniteHttp.get('shop')
            .then((res) => {
                this.setState({ itemShop: res.data.featured.concat(res.data.daily) });
                AsyncStorage.setItem('itemShop', JSON.stringify({
                    update: Date.now() + 43200000, // 12 hours
                    data: res.data.featured.concat(res.data.daily)
                }));
            })
            .catch((e) => {
                this.setState({
